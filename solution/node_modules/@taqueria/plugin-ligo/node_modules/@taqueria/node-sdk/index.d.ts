...
type CmdArgEnv = [string, string[], {
    [key: string]: string;
}];
declare const eager: <T>(f: FutureInstance<TaqError.TaqError, T>) => Promise<T>;
...
declare const spawnCmd: (fullCmd: CmdArgEnv) => Promise<number | null>;
declare const getArchSync: () => 'linux/arm64/v8' | 'linux/amd64';
declare const getArch: () => LikeAPromise<'linux/arm64/v8' | 'linux/amd64', TaqError.t>;
declare const parseJSON: <T>(input: string) => LikeAPromise<T, TaqError.TaqError>;
 ...
 */
declare const getSandboxAccountConfig: (sandbox: SandboxConfig.t, accountName: string) => ({
    encryptedKey: string;
...
declare const getContractContent: (parsedArgs: RequestArgs.t, contractFilename: string) => Promise<string | undefined>;
/**
 ...
 */
declare const getDefaultSandboxAccount: (sandbox: SandboxConfig.t) => ({
    encryptedKey: string;
    publicKeyHash: string & {
        readonly __kind: any;
    };
...
export { CmdArgEnv, Plugin, TAQ_OPERATOR_ACCOUNT, addTzExtensionIfMissing, eager, execCmd, experimental, getAccountPrivateKey, getAddressOfAlias, getArch, getArchSync, getContractContent, getContracts, getCurrentEnvironment, getCurrentEnvironmentConfig, getDefaultSandboxAccount, getDockerImage, getMetadataConfig, getNetworkConfig, getParameter, getSandboxAccountConfig, getSandboxAccountNames, getSandboxConfig, noop, parseJSON, readJsonFile, sendAsyncErr, sendAsyncJson, sendAsyncJsonErr, sendAsyncJsonRes, sendAsyncRes, sendErr, sendJson, sendJsonErr, sendJsonRes, sendRes, sendWarn, spawnCmd, stringToSHA256, updateAddressAlias, writeJsonFile };
