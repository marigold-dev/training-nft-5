...
var TaqError2 = __toESM(require("@taqueria/protocol/TaqError"));
...
var spawnCmd = (fullCmd) => new Promise((resolve, reject) => {
  const cmd = fullCmd[0];
  const args = fullCmd[1];
  const envVars = fullCmd[2];
  const child = (0, import_child_process.spawn)(cmd, args, { env: { ...process.env, ...envVars }, stdio: "inherit" });
...
var getContractContent = async (parsedArgs, contractFilename) => {
  ...
  const contractPath = (0, import_path.join)(parsedArgs.config.projectDir, parsedArgs.config.artifactsDir, contractWithTzExtension);
  try {
